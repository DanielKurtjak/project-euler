...
const UPSTREAM_FLOWING = 0;
const DOWNSTREAM_FLOWING = 1;
...
const solution = (A, B) => {
  ...
  //Propustiti upstream! posto su poslagani po upstream!
  while (i < len && B[i] === UPSTREAM_FLOWING) {
    i++;
  }
  ...
  while (i < len) {
    ...
    if (direction === DOWNSTREAM_FLOWING) {
      downstream.push(length);
    } else {
      while (downstream.length && length > downstream[downstream.length - 1]) {
        downstream.pop();
        alive--;
      }
...
console.log(solution([4, 3, 2, 1, 5], [0, 0, 1, 0, 0]));
