...
const solutionBest = a => {
  let result = 0;
  const LIMIT = 1e7;
  const len = a.length;
  const start = Array(len);
  const end = Array(len);
  for (let center = 0; center < len; center++) {
    start[center] = center - a[center];
    end[center] = center + a[center];
  }
